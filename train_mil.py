...
from nets import GatedAttention
...
parser.add_argument('-t', '--transformation', default=False, type=bool, help='Include transformations ?')
...
def train(model: nn.Module, dloader_train: DataLoader, dloader_test: DataLoader, DEVICE, optimizer, print_timing: bool=False):
    ...
    :return:
    """
    writer_folder = os.path.join('runs/writer', args.output_dir)
    ...
    for e in range(from_epoch, epoch + from_epoch):
        ...
        # Save model to file:
        if not os.path.isdir(os.path.join(writer_folder, 'Model_CheckPoints')):
            os.mkdir(os.path.join(writer_folder, 'Model_CheckPoints'))
        ...
        torch.save({'epoch': e,
                    ...
                    'acc_test': acc_test,
                    'bacc_test': bacc_test},
                   os.path.join(writer_folder, 'Model_CheckPoints', 'model_data_Epoch_' + str(e) + '.pt'))
    ...
    torch.save({'epoch': best_epoch,
               ...
               os.path.join(writer_folder, 'Model_CheckPoints', 'best_model.pt'))
...
#def check_accuracy(data_loader: DataLoader, epoch:int, train: bool = False):
    ...
    with torch.no_grad():
        ...
        """
        if train_string is not '':
            writer_string = train_string
        """
...
if __name__ == '__main__':
    ...
    if args.experiment is 0:
        args.output_dir = utils.run_data(test_fold=args.test_fold,
                                         transformations=args.test_fold,
                                         tile_size=TILE_SIZE,
    ...
    train_dset = utils.WSI_MILdataset(tile_size=TILE_SIZE,
                                      ...
                                      print_timing=False,
                                      transform=transform)
    train_loader = DataLoader(train_dset, batch_size=1, shuffle=False, num_workers=cpu_available, pin_memory=False)
    ...
    # Load model
    model = GatedAttention() #tile_size=TILE_SIZE)
    ...
    best_model, best_train_error, best_train_loss = train(model, train_loader, test_loader, DEVICE=DEVICE, optimizer=optimizer)
